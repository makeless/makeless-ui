<template>
    <b-modal :id="modalId" :ref="modalId" no-fade :title="$makeless.t('pages.token.forms.create.title')">
        <b-form id="form-token-create" @submit="onSubmit">
            <b-alert v-if="form.hasError() && form.getResponse()" variant="danger" dismissible :show="true">
                <template v-if="form.getResponse().getCode() >= 400 && form.getResponse().getCode() < 500">
                    {{ $makeless.t('pages.token.forms.create.errors.4x') }}
                </template>

                <template v-if="form.getResponse().getCode() >= 500">
                    {{ $makeless.t('pages.token.forms.create.errors.5x') }}
                </template>
            </b-alert>

            <b-form-group :label="$makeless.t('pages.token.forms.create.fields.note.label')" label-for="note">
                <b-form-input id="note" type="text" v-model="tokenCreate.note" required autocomplete="off" :placeholder="$makeless.t('pages.token.forms.create.fields.note.placeholder')"></b-form-input>
                <b-form-invalid-feedback :state="validateNote()">
                    {{ $makeless.t('pages.token.forms.create.validations.note') }}
        ...
        <template v-slot:modal-footer="{ cancel }">
            <b-button @click="cancel()">
                {{ $makeless.t('pages.token.forms.create.buttons.cancel') }}
            </b-button>
            <b-button form="form-token-create" type="submit" variant="primary" :disabled="form.isDisabled() || !validator.isValid()">
                ...
                {{ $makeless.t('pages.token.forms.create.buttons.create') }}
            </b-button>
        </template>
    </b-modal>
</template>
...
export default class Create extends Vue {
  ...
  public onSubmit($event: Event) {
    ...
    this.$makeless.getHttp().post('/api/auth/token', this.tokenCreate).then((data) => {
